/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import { Signer, utils, Contract, ContractFactory, Overrides } from "ethers";
import { Provider, TransactionRequest } from "@ethersproject/providers";
import type {
  PendleSparkLinearDiscountOracleFactory,
  PendleSparkLinearDiscountOracleFactoryInterface,
} from "../PendleSparkLinearDiscountOracleFactory";

const _abi = [
  {
    inputs: [
      {
        internalType: "address",
        name: "_pt",
        type: "address",
      },
      {
        internalType: "uint256",
        name: "_baseDiscountPerYear",
        type: "uint256",
      },
    ],
    name: "create",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "function",
  },
];

const _bytecode =
  "";

type PendleSparkLinearDiscountOracleFactoryConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: PendleSparkLinearDiscountOracleFactoryConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class PendleSparkLinearDiscountOracleFactory__factory extends ContractFactory {
  constructor(
    ...args: PendleSparkLinearDiscountOracleFactoryConstructorParams
  ) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
    this.contractName = "PendleSparkLinearDiscountOracleFactory";
  }

  deploy(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): Promise<PendleSparkLinearDiscountOracleFactory> {
    return super.deploy(
      overrides || {}
    ) as Promise<PendleSparkLinearDiscountOracleFactory>;
  }
  getDeployTransaction(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(overrides || {});
  }
  attach(address: string): PendleSparkLinearDiscountOracleFactory {
    return super.attach(address) as PendleSparkLinearDiscountOracleFactory;
  }
  connect(signer: Signer): PendleSparkLinearDiscountOracleFactory__factory {
    return super.connect(
      signer
    ) as PendleSparkLinearDiscountOracleFactory__factory;
  }
  static readonly contractName: "PendleSparkLinearDiscountOracleFactory";
  public readonly contractName: "PendleSparkLinearDiscountOracleFactory";
  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): PendleSparkLinearDiscountOracleFactoryInterface {
    return new utils.Interface(
      _abi
    ) as PendleSparkLinearDiscountOracleFactoryInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): PendleSparkLinearDiscountOracleFactory {
    return new Contract(
      address,
      _abi,
      signerOrProvider
    ) as PendleSparkLinearDiscountOracleFactory;
  }
}
